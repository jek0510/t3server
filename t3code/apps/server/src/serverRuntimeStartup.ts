    ...
    Effect.gen(function* () {
      ...
      if (serverConfig.startupPresentation === "headless") {
        ...
        if (serverConfig.mode !== "desktop") {
          ...
          // Extract token from the pairing URL hash and build a localhost URL
          // so users can paste both fields on t3code.web.app/pair
          const replitDomain = process.env["REPLIT_DEV_DOMAIN"];
          if (replitDomain) {
            let localhostPairingUrl = startupBrowserTarget;
            try {
              const parsed = new URL(startupBrowserTarget);
              const localhostBase = new URL(`http://localhost:${serverConfig.port}`);
              localhostBase.pathname = parsed.pathname;
              localhostBase.hash = parsed.hash;
              localhostPairingUrl = localhostBase.toString();
            } catch {
              // fall back to the original URL if parsing fails
            }
            const publicUrl = `https://${replitDomain}`;
            yield* Console.log(
              [
                "",
                "┌─────────────────────────────────────────────────────────┐",
